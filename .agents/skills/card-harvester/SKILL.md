...
### Step 1: Check Local CSV Datasets
Check if any new CSV files or scraped tables exist in `worker/`.
If present, run:
```bash
export NVM_DIR="$HOME/.nvm" && . "$NVM_DIR/nvm.sh" && nvm use 24.19.0 && npm run cards:sync
```

### Step 2: Web Traversal & Term Verification (If Live Search Requested)
...
### Step 4: Priority Map Update & Build Verification
1. If new cards serve as category champions, add them to `CATEGORY_PRIORITY_MAP` in `src/app/core/services/card.service.ts`.
2. Always execute production build verification:
   ```bash
